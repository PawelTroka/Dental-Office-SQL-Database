...
CREATE TABLE Klient
(PESEL_klient bigint PRIMARY KEY REFERENCES Osoba,
Typ_znizki varchar(10),
Licznik_wizyt int);

CREATE TABLE Dentysta
(Pesel_dentysta bigint PRIMARY KEY REFERENCES Osoba,
...
Id_pacjenta bigint REFERENCES Klient);
...
polozenie_szczeka_zuchwa nvarchar(15),
polozenie_lewo_prawo varchar(15),
nazwa_zeba nvarchar(10),
Id_pacjenta bigint REFERENCES Karta_pacjenta,
Id_klienta bigint REFERENCES Klient);
...
Id_dentysty bigint REFERENCES Dentysta);
...
Id_wizyty int REFERENCES Wizyta,
Id_dentysty bigint REFERENCES Dentysta,
Id_uslugi int REFERENCES Uslugi); /* z jakiej us³ugi sk³ada³ siê zabieg */
...
od_godziny TIME NOT NULL,
do_godziny TIME NOT NULL,
Id_klient bigint REFERENCES Klient, /* na termin jest zapisany klient */
Id_dentysty bigint REFERENCES Dentysta, /* do terminu przypisany jest dentysta */
Id_wizyty int REFERENCES Wizyta); /* termin przechodzi w wizytê */

/* __ Relacje wiele do wiele __ */
/* zabieg na zebie */
CREATE TABLE Wykonany_zabieg_na_zebie
(Id_zeba int REFERENCES Zab,
Id_zabiegu int REFERENCES Wykonany_zabieg);
...
/*DROP DATABASE gabinet_dentystyczny3;*/
...
/* Pokazanie wszystkich us³ug */
SELECT * FROM Uslugi;

/* Dodawanie Informacji o Us³udze */
INSERT INTO Uslugi VALUES 
('wybielanie lamp¹ beyond',310,1);

UPDATE Uslugi SET Liczba_sprzedanych = Liczba_sprzedanych+1 WHERE ID_uslugi = 14;
...
/* Dodajemy uslugi jednoczeœnie */
INSERT INTO Uslugi VALUES 
('wyrównywanie zgryzu',180,0);
...
/* Zwiêkszenie o 20% cen naszych najtañszych us³ug, tak aby podnieœæ presti¿ naszego gabinetu dentystycznego */
UPDATE Uslugi SET Cena_PLN = Cena_PLN+0.2*Cena_PLN WHERE Cena_PLN < 220;
WAITFOR DELAY '00:00:05'; 
...
/* Transakcja przeszkadzaj¹ca Transakcji 2 */
BEGIN TRANSACTION

UPDATE Uslugi SET Cena_PLN = 69.99, Nazwa = 'Nowa usluga w niskiej cenie' WHERE ID_uslugi = 1;
